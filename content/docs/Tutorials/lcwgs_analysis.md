...
```bash
nohup ./angsd_global_snp_calling.sh BAMLIST BASEDIR REFERENCE \
  MINDP MAXDP MININD MINQ MINMAF > path/output_logfile.nohup &
```
...
```bash
nohup ./get_beagle.sh BAMLIST BASEDIR REFERENCE SNPLIST > path/output_logfile.nohup &
```
...
```bash
nohup ./get_maf_per_pop.sh BASEDIR SAMPLETABLE POPCOLUMN \
  BAMLISTPREFIX REFERENCE SNPLIST MINDP MAXDP MININD MINQ \
...
```bash
nohup ./get_fst.sh SAFDIR SAMPLETABLE POPCOLUMN BASENAME > path/output_logfile.nohup &
```
...
```bash
nohup ./run_ngsadmix.sh BASEDIR BEAGLE MINMAF MINK MAXK > path/output_logfile.nohup &
```
...
```bash
nohup ./run_pcangsd.sh BASEDIR BEAGLE MINMAF admix MINE MAXE > path/output_logfile.nohup &
```
...
```bash
nohup ./run_pcangsd.sh BASEDIR BEAGLE MINMAF selection 1 1 > path/output_logfile.nohup &
```
...
```bash
nohup sh run_eems_fst.sh /path/parameterfile.ini > /path/logfiles/nohup.log &
```
...
eems.plots(
  mcmcpath, plotpath, longlat = TRUE,
